...
export function createServerInstance(config: ServerConfig): McpServer {
  ...
  server.registerTool(
    "search_awesome_items",
    {
      ...
      inputSchema: {
        query: z
          .string()
          .describe('Search keywords (e.g., "postgres ORM", "react state management")'),
        limit: z
          .number()
          .min(1)
          .max(100)
        ...
        categories: z
          .array(z.string())
          .optional()
          .describe("Filter to items tagged with these categories"),
  ...
  server.registerTool(
    "get_awesome_items",
    {
      ...
      description: "Retrieves items from a specific awesome list or section with token limiting.",
      inputSchema: {
      ...
      if (!listId && !githubRepo) {
        return {
          content: [
            {
              type: "text",
              text: "Either listId or githubRepo must be provided. Use `find_awesome_section` first.",
            },
          ],
        };
      }
